...
export async function fetchResults(searchParams: SearchParams) {
  ...
  const response = await fetch("https://realtime.oxylabs.io/v1/queries", {
    ...
    next: {
      revalidate: 60 * 60, // every 1 hr it will refresh - cache
    },
    headers: {
      Content_Type: "application/json",
      Authorization:
        "Basic " + Buffer.from(`${username}:${password}`).toString("base64"), // ensure account have access to oxylabs service
    },
  })
  ...
  console.log("Fetching results for", searchParams.url.href);
